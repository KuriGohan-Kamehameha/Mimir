...
class JapaneseTokenizer {
    ...
    private val tokenizer: Tokenizer by lazy {
        if (BuildConfig.DEBUG) Log.d(TAG, "Tokenizer: Initializing Kuromoji...")
        val t = Tokenizer()
        if (BuildConfig.DEBUG) Log.d(TAG, "Tokenizer: Ready")
        t
    }
    ...
    fun toRomaji(hiragana: String): String {
        ...
        return try {
            val transliterator = android.icu.text.Transliterator.getInstance(
                "Hiragana-Latin; NFD; [:Nonspacing Mark:] Remove; NFC"
            )
            transliterator.transliterate(hiragana)
        } catch (_: Exception) {
            hiragana
        }
    }
